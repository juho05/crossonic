...
import 'package:crossonic/features/browse/browse_page.dart';
...
import 'package:crossonic/features/songs/songs_page.dart';
...
List<RouteBase> _tabRoutes(String prefix) => [
      ...
      _newRoute(_tabRoutePath(prefix, "/browse"), const BrowsePage()),
        ...
        pageBuilder: (context, state) => NoTransitionPage(
          child: AlbumsPage(
              sortMode: AlbumSortMode.values
                  .byName(state.pathParameters["mode"] ?? "alphabetical")),
          restorationId: _tabRoutePath(prefix,
              "/albums/${state.pathParameters["mode"] ?? "alphabetical"}"),
        ),
      ),
      GoRoute(
        path: _tabRoutePath(prefix, "/songs/:mode"),
        pageBuilder: (context, state) => NoTransitionPage(
          child: SongsPage(
              sortMode: SongsSort.values
                  .byName(state.pathParameters["mode"] ?? "alphabetical")),
          restorationId: _tabRoutePath(
              prefix, "/songs/${state.pathParameters["mode"] ?? "none"}"),
        ),
      ),
      GoRoute(
    ...
    StatefulShellRoute.indexedStack(
      ...
      branches: [
        ...
        StatefulShellBranch(
          ...
          restorationScopeId: "browse_branch",
          initialLocation: "/browse",
          routes: _tabRoutes("/browse"),
        ),
        StatefulShellBranch(
