...
class FavoritesRepository extends ChangeNotifier {
  ...
  Future<Result<void>> setFavorite(
    ...
    if (favorite) {
      result = await _subsonic.star(
        _auth.con,
        ids: [if (type == FavoriteType.song) id],
        albumIds: [if (type == FavoriteType.album) id],
        artistIds: [if (type == FavoriteType.artist) id],
      );
    } else {
      result = await _subsonic.unstar(
        _auth.con,
        ids: [if (type == FavoriteType.song) id],
        albumIds: [if (type == FavoriteType.album) id],
        artistIds: [if (type == FavoriteType.artist) id],
  ...
  void updateAll(
    ...
    if (changed) {
      ...
      _db.managers.favoritesTable.bulkCreate(
        (o) => favorites.map(
          (f) => o(id: f.id, type: f.type.name, starred: f.starred!),
        ),
        mode: InsertMode.replace,
      );
      ...
      _db.managers.favoritesTable
          .filter(
            (f) =>
                (f.type(FavoriteType.song.name) &
                    f.id.isIn(notFavoriteSongIDs)) |
