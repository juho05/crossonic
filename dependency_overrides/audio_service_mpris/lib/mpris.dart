...
class OrgMprisMediaPlayer2 extends DBusObject {
  ...
  Future<DBusMethodResponse> doSeek(int offset) async {
    var newPos = Duration(microseconds: position.inMicroseconds + offset);
    if (newPos.isNegative) {
      newPos = Duration.zero;
    }
    if (metadata.length != null && metadata.length! < newPos) {
      doNext();
      return DBusMethodSuccessResponse([]);
    }
  ...
  Future<DBusMethodResponse> doSetPosition(String trackId, int position) async {
    final newPos = Duration(microseconds: position);
    if (newPos.isNegative ||
        (metadata.length != null && metadata.length! < newPos)) {
      return DBusMethodSuccessResponse([]);
    }
    _positionStreamController.add(newPos);
    return DBusMethodSuccessResponse([]);
